...
// UI 컴포넌트 생성 함수 구현 
const generateUiComponentCode = (componentType, properties = {}, parentPath = 'game.Players.LocalPlayer.PlayerGui') => {
    // 기본 컴포넌트 코드 템플릿
...
// 레이아웃 코드 생성 함수 구현
const generateLayoutCode = (layoutType, targetContainerPath, layoutOptions = {}) => {
    let layoutClassName = "";
    switch (layoutType) {
        case "List":
            layoutClassName = "UIListLayout";
            break;
        case "Grid":
            layoutClassName = "UIGridLayout";
            break;
        case "Horizontal":
            layoutClassName = "UIListLayout";
            break;
...
// 테마 코드 생성 함수 구현
const generateThemeCode = (themeName, targetRootPath) => {
...
const createComponentSchema = zod_1.z.object({
    componentType: zod_1.z.enum(['Button', 'Label', 'Frame', 'TextBox', 'ImageLabel', 'ScrollingFrame']).describe('생성할 UI 컴포넌트 유형'),
    properties: zod_1.z.record(zod_1.z.any()).optional().describe('컴포넌트의 초기 속성 (예: { Text: "Click Me", Size: new UDim2(0, 100, 0, 50) })'),
    parentPath: zod_1.z.string().optional().default('game.Players.LocalPlayer.PlayerGui').describe('부모 UI 요소의 인스턴스 경로')
});
const applyLayoutSchema = zod_1.z.object({
    layoutType: zod_1.z.enum(['List', 'Grid', 'Horizontal', 'Vertical']).describe('적용할 UI 레이아웃 유형'),
    targetContainerPath: zod_1.z.string().describe('레이아웃을 적용할 컨테이너의 인스턴스 경로'),
    layoutOptions: zod_1.z.record(zod_1.z.any()).optional().describe('레이아웃 옵션 (예: Padding, CellSize)')
});
const applyThemeSchema = zod_1.z.object({
    themeName: zod_1.z.string().describe('적용할 테마 이름 (예: Dark, Light, Fantasy)'),
    targetRootPath: zod_1.z.string().describe('테마를 적용할 루트 UI 요소의 인스턴스 경로')
});
/**
...
const uiBuilder = {
    ...
    register: (server) => {
        logger_js_1.logger.info('UI Builder 도구 등록 중...');
        // 'ui-create-component' 도구 등록
        server.tool('ui-create-component', createComponentSchema, async (params) => {
            try {
                // 검증된 매개변수 직접 사용
                ...
                logger_js_1.logger.error('UI 컴포넌트 코드 생성 오류', { err: error, params });
                return { content: [{ type: 'text', text: `UI 컴포넌트 코드 생성 오류: ${errorMessage}` }], isError: true };
            }
        });
        // 'ui-apply-layout' 도구 등록
        server.tool('ui-apply-layout', applyLayoutSchema, async (params) => {
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                logger_js_1.logger.error('UI 레이아웃 코드 생성 오류', { err: error, params });
                return { content: [{ type: 'text', text: `UI 레이아웃 코드 생성 오류: ${errorMessage}` }], isError: true };
            }
        });
        // 'ui-apply-theme' 도구 등록
        server.tool('ui-apply-theme', applyThemeSchema, async (params) => {
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                logger_js_1.logger.error('UI 테마 코드 생성 오류', { err: error, params });
                return { content: [{ type: 'text', text: `UI 테마 코드 생성 오류: ${errorMessage}` }], isError: true };
            }
        });
        logger_js_1.logger.info('UI Builder 도구가 등록되었습니다.');
    }
};
