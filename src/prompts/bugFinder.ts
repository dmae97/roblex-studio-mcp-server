...
const bugFinderSchema = z.object({
...
type BugFinderParams = z.infer<typeof bugFinderSchema>;
...
export const bugFinder = {
  register: (server: McpServer) => {
    server.prompt(
      'find-bugs',
      'Find bugs and suggest improvements in a Roblex script',
      (extra: any) => {
        let params: BugFinderParams;
        try {
          const rawParams = extra?.parameters ?? extra;
          params = bugFinderSchema.parse(rawParams);
        } catch (error) {
          logger.error('Invalid parameters received for find-bugs prompt', { error, received: extra });
          return {
            messages: [
              {
                role: 'assistant',
